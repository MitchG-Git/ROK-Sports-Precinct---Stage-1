...
| **D6-F2** | **TPS big-ticket markup** | Left as-is: ×1.15 + freight + install only, no prelims/OH/profit (~$30.1k of sell) | Pre-existing TriCore architecture — I preserved it per §5.4 rather than invent a change. But it means the grease trap, hot water units and pump set earn 15% while carrying no overhead or profit recovery. | ≈ **+$11–12k** if run through the normal cost→markup chain |
...
| **Equipment installation coverage** (HWUs, hot water pump) | Big-ticket | `Installation_MH` tokens $200–$600 | **Under test now** | M | **L** | H | H | Being resolved | Rule 4 forward-test failure if installation is only tokenised | Med | **HIGH** | Equipment test in progress; gaps to be added as crew-days |
